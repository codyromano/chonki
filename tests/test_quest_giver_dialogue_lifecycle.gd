...
func test_player_exiting_area_hides_instructions():
	...
	var instructions = quest_giver.get_node("Instructions")
	assert_almost_eq(instructions.modulate.a, 0.0, 0.1, "Instructions should fade out after player exits")
...
func test_instructions_label_can_tween_during_pause():
	var instructions = quest_giver.get_node("Instructions")
	assert_eq(instructions.process_mode, Node.PROCESS_MODE_ALWAYS,
		"Instructions label should have PROCESS_MODE_ALWAYS for tween animations during pause")
