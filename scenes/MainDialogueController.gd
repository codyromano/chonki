...
func _process(_delta: float) -> void:	
	if is_ready and rendered_dialogue and can_dismiss_dialogue and current_dialogue_choices.size() == 0 and (Input.is_action_just_pressed("ui_accept") or Input.is_action_just_pressed("ui_up")):
...
func _on_enter_warning_sign(sign_name: String) -> void:
	match sign_name:
		...
		"otter_danger":
			var dialogue_text = "WARNING: Cloud maze up ahead. Gus needs a triple jump to reach the sky."
