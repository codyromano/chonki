...
func temp_disable_collisions() -> void:
	...
	var _original_mask = collision_mask
...
func goose_disappear() -> void:
	var _chonki_body = find_parent('World2D').find_child('Chonki').find_child('ChonkiCharacter')
...
func _physics_process(delta: float) -> void:
	...
	if jump_phase == "takeoff":
		...
		velocity.y += int(GOOSE_GRAVITY * delta)
		if velocity.y >= 0:
			...
			if "is_attacking" in collider:
				...
				if collider.is_attacking():
					Utils.throttle('goose_hit', func():
						$GooseDefeated.play()
						goose_last_injured_time = int(Time.get_unix_time_from_system())
						states[GooseState.DEFEATED] = true
						goose_disappear()
					, 2)
